...
def add_part_two(fish_num):
    magnitudes = set()
    for i in range(100):
        for j in range(100):
            if i == j:
                continue
            snail_fish_i = parse(fish_num[i])
            snail_fish_j = parse(fish_num[j])
            snail_fish = add_and_reduce(snail_fish_i, snail_fish_j)
            magnitude = find_magnitude(snail_fish)
            magnitudes.add(magnitude.value)
...
print(fish_num)
print()
max_magnitude = add_part_two(fish_num)
print(max_magnitude)
